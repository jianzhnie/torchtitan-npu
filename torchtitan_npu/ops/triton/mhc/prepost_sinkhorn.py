...
@torch.library.custom_op("torchtitan_npu::mhc_pre_sinkhorn", mutates_args=(), device_types="npu")
def mhc_pre_sinkhorn_op(
    mixes: torch.Tensor,
    hc_scale: torch.Tensor,
    hc_base: torch.Tensor,
    *,
    ...
    """DeepSeek-V4 mHC pre sinkhorn: (h_pre, h_post, h_res) from mixes."""
    return hc_pre_fwd(
        mixes,
        hc_scale,
        hc_base,
...
@mhc_pre_sinkhorn_op.register_fake
def _mhc_pre_sinkhorn_fake(mixes, hc_scale, hc_base, *, hc_mult=4, sinkhorn_iters=20, eps=1e-6, group=48):
    if mixes.dim() != 3 or hc_scale.shape != (3,):
        raise ValueError("shape error in mhc_pre_sinkhorn")
    b, s, _ = mixes.shape
    comb = mixes.new_empty((b, s, hc_mult, 8))[:, :, :, :hc_mult]
...
@torch.library.custom_op("torchtitan_npu::mhc_pre_sinkhorn_bwd", mutates_args=(), device_types="npu")
def mhc_pre_sinkhorn_bwd_op(
    grad_pre: torch.Tensor,
    grad_post: torch.Tensor,
    grad_comb: torch.Tensor,
    mixes: torch.Tensor,
    ...
    *,
    ...
    """Compile-safe backward wrapper around the raw Triton kernel launches."""
    return hc_pre_bwd(
        grad_pre,
        grad_post,
        grad_comb,
...
@mhc_pre_sinkhorn_bwd_op.register_fake
def _mhc_pre_sinkhorn_bwd_fake(
    grad_pre,
    grad_post,
    grad_comb,
    mixes,
    ...
    *,
    ...
    if mixes.dim() != 3:
        raise ValueError("shape error in mhc_pre_sinkhorn_bwd")
    b, s, total_dim = mixes.shape
    return (
        torch.empty((b, s, total_dim), device=mixes.device, dtype=mixes.dtype),
        torch.empty((3,), device=mixes.device, dtype=mixes.dtype),
        torch.empty((total_dim,), device=mixes.device, dtype=mixes.dtype),
    )


def _mhc_pre_sinkhorn_setup_context(ctx, inputs, keyword_only_inputs, output):
    ctx.save_for_backward(*inputs)
    ctx.hc_mult = keyword_only_inputs["hc_mult"]
    ctx.sinkhorn_iters = keyword_only_inputs["sinkhorn_iters"]
    ctx.eps = keyword_only_inputs["eps"]


def _mhc_pre_sinkhorn_backward(ctx, grad_pre, grad_post, grad_comb):
    mixes, hc_scale, hc_base = ctx.saved_tensors
    return mhc_pre_sinkhorn_bwd_op(
        grad_pre,
        grad_post,
        grad_comb,
        mixes,
...
@torch.library.custom_op("torchtitan_npu::mhc_pre_only_sinkhorn", mutates_args=(), device_types="npu")
def mhc_pre_only_sinkhorn_op(
    mixes: torch.Tensor,
    hc_scale: torch.Tensor,
    hc_base: torch.Tensor,
    *,
    ...
    """DeepSeek-V4 mHC head sinkhorn: h_pre only, no post/comb branches."""
    return hc_pre_only_fwd(mixes, hc_scale, hc_base, hc_mult=hc_mult, eps=eps, group=group)


@mhc_pre_only_sinkhorn_op.register_fake
def _mhc_pre_only_sinkhorn_fake(mixes, hc_scale, hc_base, *, hc_mult=4, eps=1e-6, group=48):
    if mixes.dim() != 3:
        raise ValueError("shape error in mhc_pre_only_sinkhorn")
    b, s, _ = mixes.shape
    return torch.empty((b, s, hc_mult), device=mixes.device, dtype=torch.float32)


@torch.library.custom_op("torchtitan_npu::mhc_pre_only_sinkhorn_bwd", mutates_args=(), device_types="npu")
def mhc_pre_only_sinkhorn_bwd_op(
    grad_pre: torch.Tensor,
    mixes: torch.Tensor,
    hc_scale: torch.Tensor,
    hc_base: torch.Tensor,
    *,
    ...
    """Compile-safe backward wrapper around the raw Triton kernel launches."""
    return hc_pre_only_bwd(grad_pre, mixes, hc_scale, hc_base, hc_mult=hc_mult)


@mhc_pre_only_sinkhorn_bwd_op.register_fake
def _mhc_pre_only_sinkhorn_bwd_fake(grad_pre, mixes, hc_scale, hc_base, *, hc_mult=4):
    if mixes.dim() != 3:
        raise ValueError("shape error in mhc_pre_only_sinkhorn_bwd")
    b, s, total_dim = mixes.shape
    return (
        torch.empty((b, s, total_dim), device=mixes.device, dtype=torch.float32),
        torch.empty((3,), device=mixes.device, dtype=torch.float32),
        torch.empty((total_dim,), device=mixes.device, dtype=torch.float32),
    )


def _mhc_pre_only_sinkhorn_setup_context(ctx, inputs, keyword_only_inputs, output):
    ctx.save_for_backward(*inputs)
    ctx.hc_mult = keyword_only_inputs["hc_mult"]


def _mhc_pre_only_sinkhorn_backward(ctx, grad_pre):
    mixes, hc_scale, hc_base = ctx.saved_tensors
    return mhc_pre_only_sinkhorn_bwd_op(grad_pre, mixes, hc_scale, hc_base, hc_mult=ctx.hc_mult)


torch.library.register_autograd(
    mhc_pre_only_sinkhorn_op,
    _mhc_pre_only_sinkhorn_backward,
    setup_context=_mhc_pre_only_sinkhorn_setup_context,
)
