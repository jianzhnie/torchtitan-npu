...
@torch.library.custom_op("torchtitan_npu::mhc_post_bmm2", mutates_args=(), device_types="npu")
def mhc_post_bmm2_op(h_res: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    """DeepSeek-V4 mHC post bmm2: combine residual streams with h_res."""
    return hc_post_bmm2_forward(h_res, x)


@mhc_post_bmm2_op.register_fake
def _mhc_post_bmm2_fake(h_res, x):
    if h_res.ndim != 4 or x.ndim != 4:
        raise ValueError("shape error in mhc_post_bmm2")
    b, s, n, _ = h_res.shape
    c = x.shape[-1]
    return torch.empty((b, s, n, c), device=h_res.device, dtype=torch.float32)


@torch.library.custom_op("torchtitan_npu::mhc_post_bmm2_bwd", mutates_args=(), device_types="npu")
def mhc_post_bmm2_bwd_op(
    h_res: torch.Tensor, x: torch.Tensor, grad_out: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compile-safe backward wrapper around the raw Triton kernel launches."""
    return hc_post_bmm2_backward(h_res, x, grad_out)


@mhc_post_bmm2_bwd_op.register_fake
def _mhc_post_bmm2_bwd_fake(h_res, x, grad_out):
    if h_res.ndim != 4 or x.ndim != 4:
        raise ValueError("shape error in mhc_post_bmm2_bwd")
    b, s, n, _ = h_res.shape
    c = x.shape[-1]
    return (
        torch.empty((b, s, n, n), device=h_res.device, dtype=torch.float32),
        torch.empty((b, s, n, c), device=h_res.device, dtype=torch.float32),
    )


def _mhc_post_bmm2_setup_context(ctx, inputs, output):
    ctx.save_for_backward(*inputs)


def _mhc_post_bmm2_backward(ctx, grad_y):
    h_res, x = ctx.saved_tensors
    return mhc_post_bmm2_bwd_op(h_res, x, grad_y)


torch.library.register_autograd(
    mhc_post_bmm2_op,
    _mhc_post_bmm2_backward,
    setup_context=_mhc_post_bmm2_setup_context,
)
