...
@torch.library.custom_op("torchtitan_npu::mhc_pre_bmm", mutates_args=(), device_types="npu")
def mhc_pre_bmm_op(h_pre: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    """DeepSeek-V4 mHC pre bmm: weighted sum of x over the h_pre streams."""
    return hc_pre_bmm_forward(h_pre, x)


@mhc_pre_bmm_op.register_fake
def _mhc_pre_bmm_fake(h_pre, x):
    if h_pre.ndim != 3 or x.ndim != 4:
        raise ValueError("shape error in mhc_pre_bmm")
    b, s, _ = h_pre.shape
    d = x.shape[-1]
    return torch.empty((b, s, d), device=h_pre.device, dtype=torch.float32)


@torch.library.custom_op("torchtitan_npu::mhc_pre_bmm_bwd", mutates_args=(), device_types="npu")
def mhc_pre_bmm_bwd_op(
    h_pre: torch.Tensor, x: torch.Tensor, grad_out: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compile-safe backward wrapper around the raw Triton kernel launch."""
    return hc_pre_bmm_backward(h_pre, x, grad_out)


@mhc_pre_bmm_bwd_op.register_fake
def _mhc_pre_bmm_bwd_fake(h_pre, x, grad_out):
    if h_pre.ndim != 3 or x.ndim != 4:
        raise ValueError("shape error in mhc_pre_bmm_bwd")
    b, s, n = h_pre.shape
    d = x.shape[-1]
    return (
        torch.empty((b, s, n), device=h_pre.device, dtype=torch.float32),
        torch.empty((b, s, n, d), device=h_pre.device, dtype=torch.float32),
    )


def _mhc_pre_bmm_setup_context(ctx, inputs, output):
    ctx.save_for_backward(*inputs)


def _mhc_pre_bmm_backward(ctx, grad_y):
    h_pre, x = ctx.saved_tensors
    return mhc_pre_bmm_bwd_op(h_pre, x, grad_y)


torch.library.register_autograd(
    mhc_pre_bmm_op,
    _mhc_pre_bmm_backward,
    setup_context=_mhc_pre_bmm_setup_context,
)
