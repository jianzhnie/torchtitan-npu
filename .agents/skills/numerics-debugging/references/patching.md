...
Apply the patches to the TorchTitan checkout used by TorchTitan-NPU. Add
`<torchtitan-npu-root>/.agents/skills/numerics-debugging/scripts` to
`PYTHONPATH` before running so the `activation_tracer` imports below
resolve.

Always pass `<torchtitan-npu-root>/outputs/<run-name>` as `--dump_folder`.
This keeps activation logs and the generated HTML under the `outputs/`
directory next to `.agents/`, even when training runs from another checkout.
