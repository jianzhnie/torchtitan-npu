...
- **Remove from the list** if you want visibility into something you
  currently hide. Asynchronous collective comms such as
  `all_gather_into_tensor`, `all_reduce`, and `reduce_scatter_tensor`
  are excluded by default because dispatch returns before their destination
  buffers are populated; capturing them can produce false numeric diffs.
