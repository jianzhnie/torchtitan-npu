...
_DEFAULT_OUTPUT = _REPO_ROOT / "outputs" / "numerics_diff.html"
...
class HtmlReportConfig:
    """Display metadata and filtered-op summaries for one HTML report."""

    name1: str = "run1"
    name2: str = "run2"
    skipped1: frozenset[str] = field(default_factory=frozenset)
    skipped2: frozenset[str] = field(default_factory=frozenset)


@dataclass(frozen=True)
class _HashValue:
    """One rendered hash value and its optional producer keys."""
...
def _hash_intensity(e_val: str, t_val: str) -> float | None:
    ...
    if len(ev) != len(tv):
        return 1.0
    max_rd = 0.0
    for a_s, b_s in zip(ev, tv, strict=True):
...
def _format_hash_values(hash_value: _HashValue) -> str:
    """Render comma-separated hashes with producer tooltips."""
    if not hash_value.value:
        return ""
    values = [value.strip() for value in hash_value.value.split(",")]
    producers = (
        [producer.strip() for producer in hash_value.producers.split(";")]
        if hash_value.producers
        else []
    )
    parts: list[str] = []
    for index, value in enumerate(values):
        producer = producers[index] if index < len(producers) else ""
        if producer:
            tip = html.escape(f"produced by {producer}")
            parts.append(
...
def _wrap_collapsible(content: str) -> str:
    """Wrap non-empty cell content in the report's expandable container."""
    if not content:
        return ""
    return (
...
def _hash_cells(
    ...
    """Render one hash column for both sides of an operation pair."""
    first_inner = _format_hash_values(first)
    second_inner = _format_hash_values(second)
    if collapsible:
        first_inner = _wrap_collapsible(first_inner)
        second_inner = _wrap_collapsible(second_inner)
    if not paired:
        return f"<td>{first_inner}</td>", f"<td>{second_inner}</td>"

    relative_diff = _hash_intensity(first.value, second.value)
    if relative_diff is None or relative_diff == 0.0:
        return f"<td>{first_inner}</td>", f"<td>{second_inner}</td>"
    if relative_diff >= 0.1:
        intensity = 1.0
    elif relative_diff <= 1e-8:
        intensity = 0.0
    else:
        intensity = (math.log10(relative_diff) + 8) / 7
    red = int(220 + (248 - 220) * intensity)
    green = int(220 - (220 - 80) * intensity)
    blue = int(220 - (220 - 80) * intensity)
    background_alpha = intensity * 0.15
    style = (
        f' style="color: rgb({red},{green},{blue});'
        f" background: rgba(248,80,80,{background_alpha:.2f});"
        f' font-weight: bold"'
    )
    return (
        f"<td{style}>{first_inner}</td>",
        f"<td{style}>{second_inner}</td>",
    )
    ...
    for stat in STAT_FIELDS:
        ...
        if ev != tv:
            if ev is None or tv is None:
                # A stat present on only one side is a definite mismatch.
                diffs[stat] = 1.0
            else:
                rd = _rel_diff(ev, tv)
...
def generate_html(
    ...
    * Hover-highlighting that tints both halves of a row pair together.
    ...
    Args:
        ...
        config: Display names and excluded-op summaries for both runs.

    Returns:
        Complete HTML document as a string.
    """
    config = config or HtmlReportConfig()
    name1 = config.name1
    name2 = config.name2
    skipped1 = config.skipped1
    skipped2 = config.skipped2
    ...
    def _skipped_block(label: str, ops: frozenset[str]) -> str:
        if not ops:
    ...
    def make_table(entries, section_id):
        ...
        for idx, (e, t, status, diffs, strategy) in enumerate(entries):
            ...
            paired = e is not None and t is not None
            e_oh_cell, t_oh_cell = _hash_cells(
                _HashValue(e_oh),
                _HashValue(t_oh),
                paired,
                collapsible=True,
            ...
            e_ih_cell, t_ih_cell = _hash_cells(
                _HashValue(e_ih, e_ip),
                _HashValue(t_ih, t_ip),
                paired,
                collapsible=True,
            )
...
def main():
    ...
    Invoked directly from this skill's ``scripts/`` directory. See the
    ...
    parser.add_argument(
        ...
        help=f"Output HTML file (default: {_DEFAULT_OUTPUT})",
    )
    parser.add_argument(
        "--name1",
        default="run1",
    ...
    html_content = generate_html(
        ...
        HtmlReportConfig(
            name1=args.name1,
            name2=args.name2,
            skipped1=frozenset(skipped_eager),
            skipped2=frozenset(skipped_traced),
        ),
    )
    ...
    _LOGGER.info("Wrote %s (%d entries)", output_path, len(results))
