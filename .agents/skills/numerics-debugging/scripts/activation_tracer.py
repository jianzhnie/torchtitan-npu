...
class _TensorIdTracker:
    """Assign stable IDs without retaining tensors or using DebugMode internals."""

    def __init__(self) -> None:
        self._next_id = 0
        self._entries: dict[int, tuple[weakref.ReferenceType, int]] = {}

    def get_id(self, tensor: torch.Tensor) -> int:
        """Return one stable ID for the lifetime of a tensor."""
        object_id = id(tensor)
        current = self._entries.get(object_id)
        if current is not None and current[0]() is tensor:
            return current[1]

        tensor_id = self._next_id
        self._next_id += 1

        def discard(expired_ref: weakref.ReferenceType) -> None:
            current_entry = self._entries.get(object_id)
            if current_entry is not None and current_entry[0] is expired_ref:
                del self._entries[object_id]

        self._entries[object_id] = (weakref.ref(tensor, discard), tensor_id)
        return tensor_id
...
def _is_filtered_frame(path: str) -> bool:
    ...
    return "graph_trainer/debug_utils.py" in p


def _get_op_name(func) -> str:
    name = getattr(func, "name", None)
    if callable(name):
        # OpOverload.name() is public and returns e.g. aten::add.Tensor.
        return name().split("::")[-1].split(".", 1)[0]
...
def _get_current_autograd_node():
    """Return PyTorch's current autograd node through one compatibility boundary.

    PyTorch does not expose a public equivalent. Keep the dynamic lookup here so
    capture degrades to forward attribution if the internal hook moves.
    """
    torch_c = getattr(torch, "_C", None)
    get_node = getattr(torch_c, "_current_autograd_node", None)
    return get_node() if callable(get_node) else None
...
# instead of inferring the phase through ``_get_current_autograd_node()``.
...
class DebugModeTracer:
    ...
    def __enter__(self) -> dict[str, CapturedActivation]:
        ...
        def record_hook(func, types, args, kwargs, result):
            ...
            # Assign stable input/output IDs without retaining tensors. This
            # lets _materialize_captures map an input back to its producing op.

            def _id_leaves(obj):
                tree = pytree.tree_map(
                    lambda x: tensor_ids.get_id(x)
            ...
            #    is silent.
            autograd_node = _get_current_autograd_node()
            ...
            if fqn is None and debug_mode.current_nn_module_stack:
                ...
                if fqn == root:
                    ...
                    fqn = fqn[len(root) + 1:]
            if fqn is None and autograd_node is not None:
        ...
        try:
            # ExitStack preserves the required reverse teardown order and
            # still exits the remaining contexts if one __exit__ raises.
            with ExitStack() as context_stack:
                context_stack.enter_context(self._debug_mode)
                context_stack.enter_context(self._hash_ctx)
                context_stack.enter_context(self._hook_ctx)
    ...
    def is_active(self) -> bool:
        """Whether the underlying DebugMode context is currently entered."""
        return self._debug_mode is not None
    ...
    def _materialize_captures(self) -> None:
        ...
        # Map from the dispatch-time tensor ID to the capture key of
        # the op that produced it. Populated as we
