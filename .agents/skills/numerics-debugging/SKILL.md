...
description: Capture and compare per-op activations between two TorchTitan or TorchTitan-NPU runs to spot numerics divergence (eager vs aot_fx_trace, FSDP vs no-FSDP, before vs after a refactor). Use when the user wants to debug bitwise / numeric drift in training, or invokes /numerics-debugging.
---
...
Two reusable scripts live in this skill's `scripts/` directory:

- `scripts/activation_tracer.py` — runtime capture, driven from `Profiler` by
  `ActivationCaptureProfiler`. Output:
  `{dump_folder}/numerics/rank_{N}_activations.log`.
- `scripts/compare_numerics.py` — diffs two logs and produces an HTML report.

The scripts do not patch core TorchTitan or `graph_trainer` automatically.
Add this skill's `scripts/` directory to `PYTHONPATH`, edit TorchTitan to
wire the tooling in before a capture run, then revert those temporary edits
when done.

Store every generated artifact under `<torchtitan-npu-root>/outputs/`, which
is created at runtime and is not committed. Resolve the TorchTitan-NPU
repository root before running from a separate TorchTitan checkout; do not use
that checkout's local `outputs/` directory.
...
2. **Capture twice**, once per run you want to compare:
   ```bash
   TORCHTITAN_NPU_ROOT=/path/to/torchtitan-npu
   NUMERICS_SKILL_SCRIPTS="${TORCHTITAN_NPU_ROOT}/.agents/skills/numerics-debugging/scripts"
   export PYTHONPATH="${NUMERICS_SKILL_SCRIPTS}:${PYTHONPATH:-}"

   ./run_train.sh \
       --dump_folder "${TORCHTITAN_NPU_ROOT}/outputs/run_A" \
       --training.steps 2 \
...
3. **Diff** the two logs:
   ```bash
   python "${TORCHTITAN_NPU_ROOT}/.agents/skills/numerics-debugging/scripts/compare_numerics.py" \
       "${TORCHTITAN_NPU_ROOT}/outputs/run_A/numerics/rank_0_activations.log" \
       "${TORCHTITAN_NPU_ROOT}/outputs/run_B/numerics/rank_0_activations.log" \
       --name1 run_A --name2 run_B \
       -o "${TORCHTITAN_NPU_ROOT}/outputs/numerics_diff.html"
   ```
   If `-o` is omitted, the comparison tool defaults to the same repository-root
   `outputs/numerics_diff.html`. Open that report. Each row pairs one op from
   each run; cells turn red
